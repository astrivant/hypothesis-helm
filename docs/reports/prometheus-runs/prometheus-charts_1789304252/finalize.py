...
summary.extend(
    [
        "",
        "Generated C0/C1 controls are excluded except tab, LF, and CR. Other Unicode text remains eligible; "
...
sampled_counterexamples = [
    phase["values"] for record in records for phase in record.get("phases", []) if isinstance(phase.get("values"), dict)
]
assert all(supported_generated_text(values) for values in sampled_counterexamples), "Excluded controls reached sampled counterexamples"
