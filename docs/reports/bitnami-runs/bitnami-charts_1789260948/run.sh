...
parallel --will-cite --jobs 4 --null --joblog "$run_dir/joblog.tsv" \
  'HELM_CACHE_HOME=/Users/emmadoyle/projects/personal/hypothesis-helm/docs/reports/bitnami-runs/bitnami-charts_1789260948/helm/cache-{%} .venv/bin/hypothesis-helm scan {} --helm /tmp/hh-helm4/darwin-arm64/helm --chart-timeout 5m --filter --max-examples 100 --seed 0 --artifact-dir '"$run_dir"'/runs > '"$run_dir"'/jobs/{#}.json 2> '"$run_dir"'/jobs/{#}.err' \
  :::: "$run_dir/charts.txt"
status=$?
