...
(root / "previous-artifact-inventory.json").write_text(json.dumps([f"docs/benchmarks/{name}" for name in ledger], indent=2) + "\n")
Path(".cache/latest-refresh.txt").write_text(str(root) + "\n")
Path(".cache/benchmark-refresh-state.json").write_text(json.dumps({"root": str(root), "stamp": stamp}, indent=2) + "\n")
