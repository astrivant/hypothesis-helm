"""
Fit a scaled two-factor quadratic to observed cell means without extrapolating.
"""
...
class Quadratic:
    ...
    Attributes:
        coefficients (tuple[float, ...]): Intercept, u, v, uv, u squared, v squared coefficients.
        ...
        residual_sd (float): Residual deviation using cells minus six degrees of freedom.
    ...
    def predict(self, x: float, y: float) -> float:
        ...
        return sum(coefficient * term for coefficient, term in zip(self.coefficients, (1, u, v, u * v, u * u, v * v), strict=True))


def fit(points: list[tuple[float, float, float]]) -> Quadratic:
    """
    Estimate six coefficients by equal-cell ordinary least squares.

    Args:
        points (list[tuple[float, float, float]]): Unique x, y cells and their mean measured responses.

    Returns:
        Quadratic: Full-rank fit with descriptive residual diagnostics.
    """
    if len(points) <= 6 or not all(math.isfinite(value) for point in points for value in point):
        raise ValueError("need more than six finite cells to fit and assess six coefficients")
    ...
    if rank != 6:
        raise ValueError("factor settings cannot identify all six quadratic coefficients")
