...
def write_faults(path: Path, defects: list[Fault]) -> None:
    ...
    for defect in defects:
        terms = " ".join(
            f"(eq .Values.{key} {str(value).lower()})" for key, value in defect.terms.items()
        )
        lines.append(
            f'  {defect.name}: "{{{{ if and {terms} }}}}incorrect'
            '{{ else }}expected{{ end }}"'
        )
...
def select_faults(
    complexity: int, orders: tuple[int, ...], percent: float, seed: int, limit: int
) -> tuple[list[Fault], dict[str, object]]:
    """
    ...
    if (
        ...
        or not all(1 <= k <= complexity for k in orders)
    ):
            ...
            result.append(
                Fault(
                    f"bug{len(result):04d}",
                    {
                        ...
                        for position, bit in enumerate(subset)
                    },
                )
            )
    return result, {
