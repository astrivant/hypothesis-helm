...
usage() {
  cat <<'EOF'
Usage: bash benchmarks/shards.sh [options] SUITE [-- RUN_OPTIONS...]
...
The default output is a fresh directory under benchmarks/runs/local-shards/.
EOF
}
...
if [[ -n $output ]]; then
  ...
  mkdir -p benchmarks/runs/local-shards
  output=$(mktemp -d benchmarks/runs/local-shards/run-XXXXXXXX)
fi
indices=()
