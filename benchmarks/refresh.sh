...
if [[ "${1:-}" == --help ]]; then
  ...
  echo 'Runs all checks, fifteen benchmark studies (9m ceilings), plots/tables, then Bitnami and Prometheus tests (5m/chart).'
  exit 0
fi
if (($#)); then
...
if [[ -f benchmarks/runs/latest-refresh.txt ]]; then
  previous="$(cat benchmarks/runs/latest-refresh.txt)"
  if [[ -d "$previous" && ! -f "$previous/publication-finished-epoch.txt" ]]; then
    ...
    echo 'After a stopped run has been investigated, remove benchmarks/runs/latest-refresh.txt to start fresh.' >&2
    exit 2
  fi
fi
if ! mkdir benchmarks/runs/full-refresh.lock 2>/dev/null; then
  echo 'Another full refresh holds benchmarks/runs/full-refresh.lock.' >&2
  exit 2
fi
trap 'rmdir benchmarks/runs/full-refresh.lock' EXIT
printf '%s\n' 'Running project checks and tests.'
bash scripts/check.sh
git submodule update --init --recursive
root="benchmarks/runs/refresh-$(date +%s)"
python benchmarks/refresh/initialize.py "$root"
